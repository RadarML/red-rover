...
def _plot_speed(axs, tproc, traw, vproc, vraw):
    """Plot speed report."""
    for ax, vp, vr in zip(axs, vproc, vraw):
        ax.plot(tproc, vp, label="Processed")
        ax.plot(traw, vr, label="Raw")
    axs[-1].plot(tproc, np.linalg.norm(vproc, axis=0), label="Processed")
    axs[-1].plot(traw, np.linalg.norm(vraw, axis=0), label="Raw")
...
def _main(args):
    ...
    radarpose = np.load("data/test1/_slam/radar.npz")
    ...
    with PdfPages(args.out) as pdf:
        for i in tqdm(range(math.ceil(duration / args.width / 2))):
            ...
            for j, axs in enumerate([page[:4], page[4:]]):
                _plot_speed(axs, tproc, traw, vproc, vraw)
                for ax in axs:
                    ax.set_xlim(
